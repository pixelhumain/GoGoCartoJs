	...
	module.exports = L.Class.extend({
		...
		createStep: function(text, distance, icon, steps) {
			var row = L.DomUtil.create('tr', '', steps),
				span,
				td;
			td = L.DomUtil.create('td', 'td-leaflet-routing-icon', row);
			span = L.DomUtil.create('span', 'leaflet-routing-icon leaflet-routing-icon-'+icon, td);
			td.appendChild(span);
			td = L.DomUtil.create('td', 'td-leaflet-routing-text', row);
			td.appendChild(document.createTextNode(text));
			td = L.DomUtil.create('td', 'td-leaflet-routing-distance', row);
...
(function() {
	...
	module.exports = L.Control.extend({
		...
		options: {
			...
			summaryTemplate: '<h2>{time} <span class="distance">({distance})</span></h2><h3>{name}</h3>',
			timeTemplate: '{time}',
			containerClassName: '',
